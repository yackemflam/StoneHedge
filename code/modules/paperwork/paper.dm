...
/obj/item/paper/MiddleClick(mob/user)
	...
	if(hand_item)
		if(istype(hand_item, /obj/item/natural/thorn) || istype(hand_item, /obj/item/natural/feather))
			var/input_name = input("Name the parchment:")
			if(input_name)
				src.name = input_name
	else
		to_chat(user, span_warning("I need a thorn or feather to rename this parchment."))
