...
/obj/structure/roguemachine/titan/proc/give_tax_popup(mob/living/carbon/human/user)
	...
	if(newtax)
		...
		priority_announce("The new tax in the Realm shall be [newtax] percent.", "The Generous Monarch Decrees", pick('sound/misc/royal_decree.ogg', 'sound/misc/royal_decree2.ogg'), "Commander")
...
/proc/make_outlaw(raw_message, silent = FALSE)
	if(raw_message in GLOB.outlawed_players)
		...
		if(!silent)
			priority_announce("[raw_message] is no longer an outlaw in the Realm.", "The Monarch Decrees", 'sound/misc/royal_decree.ogg', "Commander")
		return FALSE
	var/found = FALSE
	...
	GLOB.outlawed_players += raw_message
	if(!silent)
		priority_announce("[raw_message] has been declared an outlaw and must be captured or slain.", "The Monarch Decrees", 'sound/misc/royal_decree2.ogg', "Captain")
	return TRUE
