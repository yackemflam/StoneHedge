...
/obj/item/mmi/attack_self(mob/user)
	if(!brain)
		radio.on = !radio.on
		to_chat(user, span_notice("I toggle [src]'s radio system [radio.on==1 ? "on" : "off"]."))
	else
		eject_brain(user)
		update_icon()
		name = initial(name)
		to_chat(user, span_notice("I unlock and upend [src], spilling the brain onto the floor."))
