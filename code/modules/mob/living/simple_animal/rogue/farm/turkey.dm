/mob/living/simple_animal/hostile/retaliate/rogue/turkey
...
/mob/living/simple_animal/hostile/retaliate/rogue/turkey/get_sound(input)
	switch(input)
		if("pain")
...
/mob/living/simple_animal/hostile/retaliate/rogue/turkey/simple_limb_hit(zone)
	if(!zone)
		return ""
	switch(zone)
...
/mob/living/simple_animal/hostile/retaliate/rogue/turkey/Initialize()
	. = ..()
	// if(!body_color)
...
/mob/living/simple_animal/hostile/retaliate/rogue/turkey/Destroy()
	--turkey_count
	return ..()

/mob/living/simple_animal/hostile/retaliate/rogue/turkey/Life()
	..()
	if(!stat && (production > 29) && egg_type && isturf(loc) && !enemies.len)
		...
		if(locate(/obj/structure/fluff/nest) in loc)
			...
			E.pixel_y = rand(-6,6)
			if(eggsFertile)
				if(turkey_count < MAX_TURKEYS && prob(50))
					E.fertile = TRUE
