...
/mob/living/carbon/human/proc/should_target(mob/living/L, ignore_lying = TRUE)
	if(!L)
		return FALSE

	//those are here for proc dependancy.
	if(!ignore_lying && L.lying && !L.get_active_held_item()) //laying with no items in hand, no threat.
