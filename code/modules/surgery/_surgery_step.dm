...
/datum/surgery_step/proc/failure(mob/user, mob/living/target, target_zone, obj/item/tool, datum/intent/intent, success_prob)
	...
	switch (success_prob)
		if (0 to 15)
			target.reagents.add_reagent(/datum/reagent/toxin/infection/major, rand(2,5))
		if (16 to 50)
			target.reagents.add_reagent(/datum/reagent/toxin/infection, rand(1,3))
		if (51 to 70)
			target.reagents.add_reagent(/datum/reagent/toxin/infection/minor, rand(1,6))
	return TRUE
