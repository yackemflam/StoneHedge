	...
	if(can_repair)
		...
		if(I.sewrepair && I.max_integrity)
			...
			if(prob(60 - skill)) //The more knowlegeable we are the easier to repair the object
				...
				user.mind.adjust_experience(/datum/skill/misc/sewing, ((user.STAINT) / 2 * (/datum/mind/proc/get_skill_speed_modifier(/datum/skill/misc/sewing))) // Only failing a repair teaches us something
				return
			else
